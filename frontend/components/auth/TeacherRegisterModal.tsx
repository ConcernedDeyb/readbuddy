...
export function TeacherRegisterModal({ open, onClose, onSuccess }: TeacherRegisterModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!email.trim().endsWith('@smccnasipit.edu.ph')) {
      setError('You must use an @smccnasipit.edu.ph email address.');
      return;
    }

    try {
      await apiFetch('/auth/teacher/register', {
        method: 'POST',
        body: JSON.stringify({
        ...
        {submitted ? (
          <div className="text-center py-2 rb-fade-in-up">
            ...
            <p className="text-xs text-gray-600 font-sans mb-5 leading-relaxed">
              Once approved by an SMCC System Administrator, you will receive an email to verify your account at <strong className="text-[#1F4D3A]">{email}</strong>.
            </p>
            <button
              type="button"
              onClick={() => {
