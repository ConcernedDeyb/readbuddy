...
async def register_teacher(body: TeacherRegisterRequest, db: AsyncSession = Depends(get_db)):
    if not body.email.endswith("@smccnasipit.edu.ph"):
        raise HTTPException(status_code=400, detail="Teacher email must be a valid @smccnasipit.edu.ph address.")
    ...
    return {"detail": "Registration submitted. An admin must approve your account before you receive a verification code."}
...
async def login_admin(body: AdminLoginRequest, response: Response, db: AsyncSession = Depends(get_db)):
    from app.models import Admin
    ident = body.username.strip()
    admin = await db.scalar(select(Admin).where(Admin.username == ident))
    if admin and verify_password(body.password, admin.password_hash):
        token = create_session_token(user_id=str(admin.id), role="admin")
        response.set_cookie("session_token", token, httponly=True, samesite="lax", max_age=60 * 60 * 24 * 7)
...
async def list_my_students(
    ...
    result = await db.execute(select(Student, Class.name.label("class_name"))
                              .outerjoin(Class, Student.class_id == Class.id)
                              .where(Student.teacher_id == teacher_session["sub"]))
    students_with_class = result.all()
    ...
    return [
        ...
        for s, class_name in students_with_class
    ]
